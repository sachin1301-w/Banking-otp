...
export default function Deposit() {
  ...
  const requestOtp = async (payload, isResend = false) => {
    ...
    setOtpError("");
    try {
      const response = await transactionsApi.requestDepositOtp(payload);
      setChallenge({ ...response, payload });
      setStatus({ state: "success", message: response.message || "OTP sent. Verify it to complete the deposit." });
    } catch (err) {
  ...
  const verify = async (otp) => {
    ...
    try {
      ...
      setChallenge(null);
      setStatus({ state: "success", message: `Deposit completed. Reference: ${tx.referenceId || tx.id}` });
      setAmount("");
      await reload();
    } catch (err) {
  ...
  return (
    <>
      <MoneyOperationCard
        ...
        description="For your security, we email a 6-digit OTP before any deposit is completed. Your balance changes only after OTP verification."
        accounts={accounts}
        selected={selected}
        onSelected={setSelected}
        amount={amount}
        onAmount={setAmount}
        submitLabel="Send OTP & deposit"
