...
export default function Withdraw() {
  ...
  const requestOtp = async (payload, isResend = false) => {
    ...
    setOtpError("");
    try {
      const response = await transactionsApi.requestWithdrawOtp(payload);
      setChallenge({ ...response, payload });
      setStatus({ state: "success", message: response.message || "OTP sent. Verify it to complete the withdrawal." });
    } catch (err) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    await requestOtp({ accountNumber: selected, amount: Number(amount) });
  };
  ...
  const verify = async (otp) => {
    ...
    try {
      ...
      setChallenge(null);
      setStatus({ state: "success", message: `Withdrawal completed. Reference: ${tx.referenceId || tx.id}` });
      setAmount("");
      await reload();
    } catch (err) {
  ...
  return (
    <>
      <MoneyOperationCard
        ...
        description="A 6-digit OTP is sent to your registered email before the withdrawal is approved. Your balance is unchanged until verification."
        accounts={accounts}
        selected={selected}
        onSelected={setSelected}
        amount={amount}
        onAmount={setAmount}
        submitLabel="Send OTP & withdraw"
